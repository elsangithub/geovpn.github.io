...
clear
echo -e "[ ${green}INFO${NC} ] Creat Github Repostory & Creat Akses Token"
echo -e "[ ${green}INFO${NC} ] Create password for database"
read -rp "Enter Email Git : " -e Email
read -rp "Enter Username Git : " -e Git
read -rp "Enter Your Repo Backup Usee : " -e Repo
read -rp "Enter Akses Token : " -e Tok
...
Save_And_Exit () {
    ...
    git config --global user.name "$Git" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    ...
    git push -f https://$Tok@$Repo &> /dev/null
}
